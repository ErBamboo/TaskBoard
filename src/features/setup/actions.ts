...
function readSerializedArrayLength(
  formData: FormData,
  fieldName: string,
) {
  ...
  try {
    ...
    return Array.isArray(parsedValue) ? parsedValue.length : null;
  } catch {
    return null;
  }
}
...
export async function initializeWorkspaceAction(
  ...
  try {
    ...
    console.error("[initializeWorkspaceAction] unexpected failure", {
      error,
      memberCount: readSerializedArrayLength(formData, "members"),
      projectCount: readSerializedArrayLength(formData, "projects"),
      subsystemTemplateCount: readSerializedArrayLength(
        formData,
        "subsystemTemplates",
      ),
      teamName:
        typeof formData.get("teamName") === "string"
